...
GLint a3option = 0; //animation option for a3,  0:for selected, 1:simple solar, 2:solar system
GLint a4option = 0; //display optoin for a4,  0:for control point section, 1:Bezier curve generation, 2: rotation surface, 3:for solar system
...
void mouseMotion(GLint x, GLint y) {
	if (moving) {
		if (coordinate == 1){      //model transformations
			modelTransforms(x);
		}
		else if (coordinate == 2){ //WC transformations
			worldTransforms(x);
		}
		else if(coordinate == 3){ //view transformations
			viewTransforms(x);
		}
